...
using Qoollo.Impl.DbController.Model;
using Qoollo.Impl.Modules;
using Qoollo.Impl.Modules.Async;
using Qoollo.Impl.Modules.Queue;

namespace Qoollo.Impl.DbController.Distributor
{
    internal class DistributorModule : ControlModule
    {
        private ControllerModel _model;
        private DbControllerNetModule _dbControllerNet;
        ...
        public DistributorModule(AsyncTaskModule async, AsyncDbWorkModule asyncDbWork,
            DbControllerNetModule dbControllerNet,
            ServerId local,
            ...
            AsyncTasksConfiguration pingConfiguration = null)
        {
            ...
            _model = new ControllerModel(local, hashMapConfiguration);
            _dbControllerNet = dbControllerNet;
        ...
        {
            var servers = _dbControllerNet.GetServersByType(typeof (SingleConnectionToDistributor));
            _dbControllerNet.PingDistributors(servers);

            servers = _dbControllerNet.GetServersByType(typeof(SingleConnectionToController));
            _dbControllerNet.PingControllers(servers);
        }
            ...
            string tableName)
        {
            ...
            if (!_dbControllerNet.ConnectToDistributor(server))
        ...
        {
            if (command is RestoreCommand)
                _dbControllerNet.SendToDistributor(command);
        }

        private void ProcessTransaction(Transaction transaction)
        {
            if (transaction.Distributor != null)
                _dbControllerNet.TransactionAnswer(transaction.Distributor, transaction);
        }
