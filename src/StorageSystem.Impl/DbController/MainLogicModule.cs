...
namespace Qoollo.Impl.DbController
{
    internal class MainLogicModule:ControlModule
    {
        ...
        public RemoteResult Process(InnerData data)
        {
            ...
            switch (data.Transaction.OperationName)
            {
                case OperationName.Create:
                    ...
                    PerfCounters.DbControllerCounters.Instance.CreatePerSec.OperationFinished();
                    break;
                case OperationName.Custom:
                    CheckResult(data, _db.CustomOperation(data, local));
                    PerfCounters.DbControllerCounters.Instance.CustomOperationPerSec.OperationFinished();
                    break;
                case OperationName.Delete:
                    CheckResult(data, _db.Delete(data));
                    PerfCounters.DbControllerCounters.Instance.DeletePerSec.OperationFinished();
                    break;
                case OperationName.RestoreUpdate:
                    ret = _db.RestoreUpdate(data, local);
                    PerfCounters.DbControllerCounters.Instance.RestoreUpdatePerSec.OperationFinished();
                    break;
                case OperationName.Update:
                    CheckResult(data, _db.Update(data, local));
                    PerfCounters.DbControllerCounters.Instance.UpdatePerSec.OperationFinished();
                    break;
            }
        ...
        public InnerData Read(InnerData data)
        {
            ...
            PerfCounters.DbControllerCounters.Instance.ReadPerSec.OperationFinished();
            return ret;
        }
